...
class PlanPeriodization:
    ...
    # Applies periodization model to training plan: linear, wave, block

    @staticmethod
    def apply_linear(week: int) -> Dict:
        return {
            "intensity": 0.6 + week * 0.05,
            "volume": 1.0
        }
    ...
    def apply_wave(week: int) -> Dict:
        ...
        return {
            "intensity": wave[(week - 1) % len(wave)],
            "volume": 1.0
        }
    ...
    def apply_block(week: int) -> Dict:
        ...
        if week == 4:
            return {"intensity": 0.5, "volume": 0.8}  # deload
